...
class FormRegistro(FlaskForm):
    usuario = StringField('Usuario',validators=[validators.required(),validators.length(max=20),validators.DataRequired(message="No deje vacio este campo, esciba el usuario")])
    password = StringField('Password',validators=[validators.required(),validators.length(max=50),validators.DataRequired(message="No deje vacio este campo, esciba la contraseña")])
    repassword = StringField("Confirmar Password", validators=[validators.required(),validators.length(max=50),validators.DataRequired(message="No deje vacio este campo, re escriba la contraseña")])
